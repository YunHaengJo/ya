      ...
      <div class="row">
        ...
        <tr>
         ...
         <input type="hidden" name="command" value="memberlist">
        	...
        	<button class="input-group-addon" id="write" style="width: 356px; height: 30px; background-color: #ededed;" onclick="mySubmit(1)">문의하기</button>
            <button class="input-group-addon" style="width: 356px; height: 30px; background-color: white;" onclick="mySubmit(2)">나의 문의내역</button>
        ...
        <tr>
        	...
        	if(null != list && list.size() > 0){
        		...
        		<%	for(int i=0; i<pageCnt; i++){
	        			if(i+1 == pageNum){
	        			%>	<a href="qnaControl.jsp?command=memberlist&pageNum=<%=i+1%>&email=<%=user.getEmail()%>&search_word=<%=search_word %>&search_div=<%=search_div %>" style="color: red;"><%=i+1%></a>
	        		<%	}
	        			else
	        			{%>
	        				<a href="qnaControl.jsp?command=memberlist&pageNum=<%=i+1%>&email=<%=user.getEmail() %>&search_word=<%=search_word %>&search_div=<%=search_div %>" style="color: black;"><%=i+1%></a>
	        		<%	}%>
        		<%	}
        			if(pageCnt != 1){
        		%>	<a href="qnaControl.jsp?command=memberlist&pageNum=<%=pageNum+1%>&email=<%=user.getEmail() %>&search_word=<%=search_word %>&search_div=<%=search_div %>"><span class="glyphicon glyphicon-chevron-right" style="color: red; width: 30px; height: 20px"></span></a>
        		<%	}		
        		}
        		...
        		%>
        			<a href="qnaControl.jsp?command=memberlist&pageNum=<%=pageNum-1%>&email=<%=user.getEmail() %>&search_word=<%=search_word %>&search_div=<%=search_div %>"><span class="glyphicon glyphicon-chevron-left" style="color: red; width: 30px; height: 20px"></span></a>
        	...
        	<%	}
        			...
        			<a href="qnaControl.jsp?command=memberlist&pageNum=<%=pageNum-1%>&email=<%=user.getEmail() %>&search_word=<%=search_word %>&search_div=<%=search_div %>">
        ...
        <form action="./qnaControl.jsp?command=memberlist" method="post"> 
